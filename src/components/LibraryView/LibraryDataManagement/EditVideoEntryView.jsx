...
export const EditVideoEntryView = ({
  update_element_in_library,
  video_entry,
}) => {
  if (!video_entry) {
  ...
  async function get_tmdb_covers() {
    if (tmdb_cover_results.length === 0) {
      ...
      await invoke("get_covers_from_tmdb", {
        tmdb_id: video_entry.tmdb_id,
      })
        .then((poster_paths) => {
  ...
  return (
    <div>
      ...
      <div className={editVideoStyles.container}>
        {/* Cover */}
        <div
          className={editVideoStyles.containerBox}

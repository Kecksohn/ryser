...
fn start_process(filepath: &str, args: &str,  state: State<Arc<ProcessManager>>) -> Option<u32> {
...
pub fn start_video_in_vlc(filepath: &str, state: State<Arc<ProcessManager>>) -> Option<u32> {
    return start_process("C:/Program Files/VideoLAN/VLC/vlc.exe", filepath, state);
}
...
pub fn start_video_in_mpc(filepath: &str, state: State<Arc<ProcessManager>>) -> Option<u32> {
    return start_process("C:/Program Files (x86)/K-Lite Codec Pack/MPC-HC64/mpc-hc64.exe", &(" ".to_owned() + filepath), state);
}
